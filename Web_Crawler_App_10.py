...
def crawl_category(category_url, main_domain, max_pages=10, keywords=None):
    """Crawl a specific category up to max_pages and return results."""
    if keywords is None:
        keywords = ["gowithguide", "go with guide", "go-with-guide", "87121"]
    ...
    while queue and page_count < max_pages:
        ...
        processed_links, soup = process_url(url, main_domain, visited, results, status)
        page_count += 1
        
        # If we found matches, stop crawling
        if results:
            status.append(("🎯", f"Found {len(results)} matches! Stopping crawl."))
            return results, status, visited
            
        # Add new links to the queue
        for link in processed_links or []:
            if link not in visited:
                queue.append(link)
                
        # Add the status message about pages crawled
        elapsed_time = time.time() - start_time
        status.append(("📊", f"Category progress: {page_count}/{max_pages} pages, {elapsed_time:.1f}s elapsed"))
        
    if not results:
        status.append(("ℹ️", f"No matches found after crawling {page_count} pages in this category."))
...
def main():
    ...
    if st.session_state.crawl_data['running']:
        ...
        if not st.session_state.crawl_data['current_category']:
            # Process URLs in optimized batches for main domain
            ...
            while st.session_state.crawl_data['running'] and st.session_state.crawl_data['pages_crawled'] < max_pages:
                if not st.session_state.crawl_data['queue']:
                    st.session_state.crawl_data['status'].append(("ℹ️", f"No more pages to crawl in main domain."))
                    break
                ...
                if st.session_state.crawl_data['pages_crawled'] >= max_pages:
                    ...
                    if not st.session_state.crawl_data['results'] and st.session_state.crawl_data['categories']:
                        ...
                        st.session_state.crawl_data['status'].append(("ℹ️", "No categories found to continue crawling."))
                        st.session_state.crawl_data['running'] = False
        ...
        # Category crawl logic
            ...
            while st.session_state.crawl_data['running'] and st.session_state.crawl_data['pages_crawled'] < max_pages:
                if not st.session_state.crawl_data['queue']:
                    st.session_state.crawl_data['status'].append(("ℹ️", f"No more pages to crawl in '{category_name}' category."))
                    break
                ...
                if st.session_state.crawl_data['pages_crawled'] >= max_pages:
                    ...
                    if current_idx < len(categories) - 1:
                        ...
                        st.session_state.crawl_data['status'].append(("🔄", f"Moving to '{next_category[0]}' category."))
                        st.session_state.crawl_data['queue'] = deque([next_category[1]])
                        st.session_state.crawl_data['pages_crawled'] = 0
                    else:
                        st.session_state.crawl_data['status'].append(("ℹ️", "No more categories to crawl."))
                        st.session_state.crawl_data['running'] = False
